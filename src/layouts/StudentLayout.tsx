...
export const StudentLayout: React.FC = () => {
  ...
  const navigationItems = [
    { name: 'overview Dashboard', path: '/student', icon: LayoutDashboard },
    { name: 'Available Courses', path: '/student/courses', icon: Search },
    { name: 'My Enrollments', path: '/student/enrollments', icon: BookOpen },
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex" id="student-layout">
      {/* 1. OFF-CANVAS MOBILE DRAWER */}
      {mobileMenuOpen && (
        <div className="fixed inset-0 z-50 lg:hidden flex" id="mobile-drawer-wrapper-student">
          {/* Backdrop */}
          <div 
          ...
          <div className="relative flex-1 flex flex-col max-w-xs w-full bg-white border-r border-slate-200 p-6 shadow-xl animate-fade-in-left">
            <div className="flex items-center justify-between mb-8" id="drawer-header-student">
              <div className="flex items-center gap-2">
                <span className="p-2 bg-blue-50 text-blue-700 rounded-lg">
                  <GraduationCap className="h-5 w-5" />
                </span>
                <span className="font-bold text-slate-800 tracking-tight text-sm">Trainee Cabinet</span>
              </div>
              <button 
                onClick={() => setMobileMenuOpen(false)}
            ...
            <div className="mb-6 p-4 bg-slate-50 rounded-xl border border-slate-100 flex items-center gap-3">
              <div className="w-10 h-10 bg-indigo-600 text-white rounded-lg font-bold flex items-center justify-center">
                {profile?.name?.charAt(0) || user?.email?.charAt(0).toUpperCase() || 'S'}
              </div>
              <div className="overflow-hidden">
                <p className="text-sm font-bold text-slate-800 truncate leading-tight">{profile?.name || 'Student'}</p>
                <span className="text-[10px] text-indigo-600 font-semibold uppercase flex items-center gap-0.5">
                  <UserCheck className="h-3 w-3 inline" /> Trainee Account
                </span>
              </div>
            </div>

            <nav className="flex-1 flex flex-col gap-1.5" id="mobile-drawer-nav-student">
      ...
      <aside className="hidden lg:flex flex-col w-64 bg-white border-r border-slate-200" id="student-desktop-sidebar">
        {/* Brand Banner */}
        <div className="h-16 px-6 border-b border-slate-200 flex items-center gap-3 shrink-0">
          ...
          <div className="leading-none">
            ...
            <span className="text-[9px] font-bold text-slate-400 tracking-wider uppercase">Student Desk</span>
          </div>
        </div>

        {/* Profile Card Summary */}
        <div className="p-4 mx-4 mt-6 mb-4 bg-slate-50 rounded-xl border border-slate-200/50 flex items-center gap-3">
          <div className="w-9 h-9 bg-indigo-600 text-white rounded shadow-sm font-bold flex items-center justify-center shrink-0">
            {profile?.name?.charAt(0) || user?.email?.charAt(0).toUpperCase() || 'S'}
          </div>
          <div className="overflow-hidden">
            <p className="text-xs font-bold text-slate-800 truncate leading-snug">{profile?.name || 'Trainee'}</p>
            <span className="text-[9px] font-semibold text-indigo-600 uppercase flex items-center gap-0.5">
              <UserCheck className="h-2.5 w-2.5" /> Registered Student
            </span>
          </div>
        </div>

        {/* Navigation list */}
        <div className="flex-1 px-4 flex flex-col gap-1.5" id="desktop-sidebar-nav-container-student">
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest px-4 mb-1">Foundation Modules</p>
          <nav className="flex flex-col gap-1" id="desktop-sidebar-nav-student">
            {navigationItems.map((item) => {
              const IconComponent = item.icon;
              return (
                <NavLink
      ...
      <div className="flex-1 flex flex-col min-w-0" id="student-content-container">
        {/* Top Header navbar */}
        <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-4 sm:px-6 lg:px-8 sticky top-0 z-10">
          <button 
            ...
            id="mobile-drawer-trigger-student"
          >
            <Menu className="h-5 w-5" />
          </button>

          <div className="hidden sm:flex items-center gap-2 text-[11px]" id="topbar-timezone-student">
          ...
          <div className="flex items-center gap-4">
            <Link 
              ...
              id="header-profile-link-student"
            >
              School ID: {profile?.role === 'student' ? (profile as any).studentId || 'UNASSIGNED' : 'UNASSIGNED'}
            </Link>
          </div>
        </header>

        {/* Sub page nested content wrapper */}
        <main className="flex-grow p-4 sm:p-6 lg:p-8 overflow-y-auto" id="student-page-viewport">
          <Outlet />
        </main>
      </div>
    </div>
